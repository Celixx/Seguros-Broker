...
namespace Seguros_Broker.Repositorio
{
    public class PropuestaRep : Repositorio
    {
        ...
        public Propuesta? GetPropuesta(int propuestaID)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
                    string sql = "SELECT * FROM PROPUESTA WHERE ID=@propuestaID";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                propuesta.NumeroPoliza = reader.IsDBNull(reader.GetOrdinal("NumeroPoliza")) ? 0 : reader.GetInt32(1);
        ...
        public Propuesta? GetPropuestaNumPoliza(int numPoliza)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
                    string sql = "SELECT * FROM PROPUESTA WHERE NumeroPoliza=@numPoliza";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {

                        command.Parameters.AddWithValue("@numPoliza", numPoliza);
        ...
        public Propuesta? GetPropuestaByNumeroPoliza(int numeroPoliza)
        {
            try
            {
                ...
                {
                    conn.Open();
                    string sql = @"SELECT TOP 1 ID, NumeroPoliza, FechaRecepcion, TipoPoliza, FechaIngreso, FechaEmision, IDRamo, IDEjecutivo,
                                      Area, FechaCreacion, FechaVigenciaDesde, FechaVigenciaHasta, IDMoneda, ComisionAfecta, ComisionExenta,
                                      MontoAsegurado, ComisionTotal, PrimaNetaAfecta, PrimaNetaExenta, PrimaNetaTotal, IVA, PrimaBrutaTotal,
